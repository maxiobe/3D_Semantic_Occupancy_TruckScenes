...
class NuScenesDataset(Dataset):
    ...
    def __init__(
        ...
        self.sensor_types = ['CAMERA_LEFT_FRONT', 'CAMERA_RIGHT_FRONT', 'CAMERA_LEFT_BACK', 'CAMERA_RIGHT_BACK']
    ...
    def get_data_info(self, info):
        ...
        lidar2ego_r = Quaternion(info['data']['LIDAR_LEFT']['calib']['rotation']).rotation_matrix
        lidar2ego = np.eye(4)
        lidar2ego[:3, :3] = lidar2ego_r
        lidar2ego[:3, 3] = np.array(info['data']['LIDAR_LEFT']['calib']['translation']).T
        ego2lidar = np.linalg.inv(lidar2ego)

        lidar2global = get_lidar2global(info['data']['LIDAR_LEFT']['calib'], info['data']['LIDAR_LEFT']['pose'])
        ego2global = np.eye(4)
        ego2global[:3, :3] = Quaternion(info['data']['LIDAR_LEFT']['pose']['rotation']).rotation_matrix
