...
OUTPUT_DIR="${BASE_PROJECT_DIR}/prediction/GaussianFormerOne/out"

RUN_NAME="run_4GPU_sbatch_a100_1"
...
PY
        ...
        conda run -n gaussianformer pip install -e /code/prediction/GaussianFormerOne/model/encoder/gaussian_encoder/ops
        conda run -n gaussianformer pip install -e /code/prediction/GaussianFormerOne/model/head/localagg
        conda run -n gaussianformer pip install -e /code/prediction/GaussianFormerOne/model/head/localagg_prob
        conda run -n gaussianformer pip install -e /code/prediction/GaussianFormerOne/model/head/localagg_prob_fast

        # pointops (clone if missing)
        if [ ! -d /code/prediction/GaussianFormerOne/installation/pointops ]; then
          echo "[Build] Cloning pointops"
          mkdir -p /code/prediction/GaussianFormerOne/installation
        ...
        conda run -n gaussianformer pip install -e /code/prediction/GaussianFormerOne/installation/pointops

	mkdir -p /output/prob/nuscenes_gs6400/'"$RUN_NAME"'

        # --- Start training (your exact command; spawns per-GPU inside) ---
        cd /code/prediction/GaussianFormerOne
