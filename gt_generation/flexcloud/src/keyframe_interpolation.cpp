  ...
  const std::string & pcd_dir)
{
  ...
  std::cout << "Sorting " << this->frames_.size() << " odometry frames by timestamp..." << std::endl;
  std::sort(
    this->frames_.begin(), this->frames_.end(),
    [](const std::shared_ptr<OdometryFrame>& a, const std::shared_ptr<OdometryFrame>& b) {
